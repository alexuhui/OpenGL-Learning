...
{	
	...
	vec3 ep0 = gl_in[0].gl_Position.xyz + varyingOriginalNormal[0]*sLen;
	vec3 ep1 = gl_in[1].gl_Position.xyz + varyingOriginalNormal[1]*sLen;
	vec3 ep2 = gl_in[2].gl_Position.xyz + varyingOriginalNormal[2]*sLen;
	...
	varyingVertPos = newPoint1;
	varyingLightDir = light.position - newPoint1;
	varyingNormal = varyingOriginalNormal[0];
	EmitVertex();
	
	gl_Position = proj_matrix * vec4(newPoint2, 1.0);
	varyingVertPos = newPoint2;
	varyingLightDir = light.position - newPoint2;
	varyingNormal = varyingOriginalNormal[1];
	EmitVertex();
