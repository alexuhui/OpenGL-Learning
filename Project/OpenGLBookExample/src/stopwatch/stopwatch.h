...
namespace stopwatch{
    ...
    public:
       ...
       Stopwatch(): start_time(), step_start_time(), laps({}) {
          start();
       }
       ...
       template<TimeFormat fmt = TimeFormat::MILLISECONDS>
       std::uint64_t stepElapsed() {
           const auto end_time = std::chrono::high_resolution_clock::now();
           const auto ori_start_time = step_start_time;
           step_start_time = end_time;
           return ticks<fmt>(ori_start_time, end_time);
       }
